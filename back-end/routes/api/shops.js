...
// @route   GET /api/shops/getFavShops
// @desc    Get Favorite Shops
// @access  Private
...
// @route   POST /api/shops/removeShop
// @desc    Remove Shop From Favorite
// @access  Private
router.post("/removeShop", [middleware.auth], async (req, res) => {
  try {
    const result = await shopModel.removeShop(req.user.id, req.body.id);
    res.json({ success: true, msg: "Remove Shop From Favorite Done" });
  } catch (error) {
    res.status(500).send("server error");
  }
});
